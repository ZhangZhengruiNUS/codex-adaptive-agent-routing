...
一条大型独立只读工作线可使用一个聚焦子代理；若两条及以上工作线可以并行推进且不
阻塞关键路径，则每条选中的工作线都必须委派一个子代理：通常两个，只有额外工作线能
提升覆盖面或速度时才扩到四个。小型或紧密耦合的工作仍由主任务完成。
...
- 模板显式授权有边界的委派：一条大型独立只读工作线可使用一个子代理；两条及以上
  选中的工作线则要求并行。主代理仍会判断工作线是否独立，因此这是基于模型判断的
  自适应路由，而不是完全确定性的调度器。
